...
//    setTempRange(2.0);
    ...
    justRebuildCoef = p->childs->configDelegate->checkBoxRebuildCoef->isChecked();
    const uint64_t freq_step = FREQ_M(100);
...
{
    Q_Cal_TXFilter_Widget *p = dynamic_cast<Q_Cal_TXFilter_Widget *>(parent());
//    QList<Qwt_TX_RF_IF_FR_0000_3000_Data *> *qwtData = &(p->childs->dataRF_IF_FR_0000_3000);
    ...
    int64_t freq_if = 0;
    int64_t freq_if_star = FREQ_M(-50);
    int64_t freq_if_stop = FREQ_M(50);

    quint32 idx_freq_rf = 0;
    ...
    for (freq_rf = tx_freq_star + BW_Max / 2;freq_rf < tx_freq_sec0;freq_rf += step) {
        THREAD_TEST_PAUSE_S
        THREAD_TEST_CANCEL
        ...
        for (quint32 i = 0;i <= idx_freq_rf;++i) {
            p->childs->dataRF_IF_FR_0000_3000[i]->table =
                &(SP1403->cal_file()->tx_rf_if_fr_0000_3000_table()->data_calibrating()->at(i));
        }
        ...
        for (quint32 i = 0;i < ARRAY_SIZE(freq_dds1);++i) {
            ...
            for (freq_if = freq_if_star;freq_if <= freq_if_stop;freq_if += step) {
                ...
                data.pts[idx_freq_if].x = uint64_t(int64_t(freq_rf) + freq_dds1_cur + freq_if);
