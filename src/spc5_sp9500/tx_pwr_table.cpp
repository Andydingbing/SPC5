...
{
    uint32_t idx = SERIE_INDEX(data->state[0].freq,RF_TX_FREQ_STAR,RF_TX_FREQ_STEP_INFILE);
    m_data_f[idx] = *data;
}
...
uint32_t tx_pwr_table_r1ab::get_pwr_idx(double pwr)
{
    ...
    return SERIE_INDEX(pwr_l,R1A_TX_POWER_STAR,R1A_TX_POWER_STEP);
}
...
//    int32_t d_gain_max = R1A_DGAIN_MAX;
    ...
    uint32_t freq_idx_l = SERIE_INDEX(freq_l,RF_TX_FREQ_STAR,RF_TX_FREQ_STEP_CALLED);
    uint32_t freq_idx_r = SERIE_INDEX(freq_r,RF_TX_FREQ_STAR,RF_TX_FREQ_STEP_CALLED);
    ...
    if (pwr >= R1A_TX_POWER_STOP + 10) {
        ...
        int32_t extra_att_l = (int32_t(R1A_TX_POWER_STOP + 10 - pwr) / 10) * 10;    //extra rf att from -20(right version),0,10,20,30...
        int32_t extra_att_r = extra_att_l + R1A_TX_ATT_STEP_MIN;
        d_gain_l = double(data_m_m.state[pwr_idx_l].d_gain) - (R1A_TX_POWER_STOP + 10 - extra_att_l - pwr);			//may < R1A_DGAIN_MIN
        if (d_gain_l < d_gain_min) {
            d_gain_r = double(data_m_m.state[pwr_idx_l].d_gain) + (pwr - (R1A_TX_POWER_STOP + 10 - extra_att_r));   //may > R1A_DGAIN_MAX
            if (d_gain_r > R1A_DGAIN_MAX)
                use_l = ((d_gain_min - d_gain_l) <= (d_gain_r - R1A_DGAIN_MAX)) ? true : false;
			else
                use_l = false;
...
//    double att1 = data_base.att1 / 2.0;
    ...
    if (pwr >= R1C_TX_BASE_POWER_OP + 5.0) {
        ...
        d_gain += (pwr - R1C_TX_BASE_POWER_OP) - (att0_copy - att0);
    } else if (pwr >= R1C_TX_BASE_POWER_OP) {
        att0 -= pwr - R1C_TX_BASE_POWER_OP;
        if (att0 < 0) {
            d_gain -= att0;
            att0 = 0;
        }
    } else if (pwr >= R1C_TX_BASE_POWER_OP - 5.0) {
        att1 = -5.0 - pwr;
    } else if (pwr >= R1C_TX_BASE_POWER_OP - 35.0) {
        att1 += 5.0;
        att2 = -10.0 - pwr;
...
void tx_pwr_op_table_r1c::save_as(const char *path)
{
    ...
    double pwr = 10.0;
    tx_pwr_op_table_r1c::data_f_t data;
...
//    double att1 = data_base.att1 / 2.0;
    ...
    if (pwr >= R1C_TX_BASE_POWER_IO + 3.0) {
        double att0_copy = att0;
        att0 -= 3.0;
        if (att0 < 0) {
            d_gain -= att0;
        ...
        d_gain += (pwr - R1C_TX_BASE_POWER_IO) - (att0_copy - att0);
    } else if (pwr >= R1C_TX_BASE_POWER_IO) {
        att0 -= pwr - R1C_TX_BASE_POWER_IO;
        if (att0 < 0) {
            d_gain -= att0;
            att0 = 0;
        }
    } else if (pwr >= R1C_TX_BASE_POWER_IO - 2.0) {
        att0 += R1C_TX_BASE_POWER_IO - pwr;
    } else if (pwr >= R1C_TX_BASE_POWER_IO - 7.0) {
        att0 += 2.0;
        att1 += R1C_TX_BASE_POWER_IO - pwr - 2.0;
    } else if (pwr >= R1C_TX_BASE_POWER_IO - 37.0) {
        att0 += 2.0;
        att1 += 5.0;
        att2 += R1C_TX_BASE_POWER_IO - pwr - 7.0;
    } else if (pwr >= R1C_TX_BASE_POWER_IO - 62.0) {
        att0 += R1C_TX_BASE_POWER_IO - pwr - 35.0;
        att1 += 5.0;
        att2 += 30.0;
    } else if (pwr >= R1C_TX_BASE_POWER_IO - 87.0) {
        att0 += 27.0;
        att1 += R1C_TX_BASE_POWER_IO - pwr - 57.0;
        att2 += 30.0;
    } else {
        att0 += 27.0;
        att1 += 30.0;
        att2 += 30.0;
        d_gain -= R1C_TX_BASE_POWER_IO - pwr - 87.0;
    }
...
void tx_pwr_io_table_r1c::save_as(const char *path)
{
    ...
    double pwr = 0.0;
    tx_pwr_io_table_r1c::data_f_t data;
