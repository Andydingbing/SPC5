...
{
    m_sp1401 = boost::make_shared<vector<basic_sp1401::sptr>>();
...
sp1401_r1e *sp3301::get_sp1401_r1e(int32_t rf_idx)
{
    return m_sp1401_r1e->at(rf_idx).get();
}

sp1401_r1f *sp3301::get_sp1401_r1f(int32_t rf_idx)
{
    return m_sp1401_r1f->at(rf_idx).get();
}
    ...
    if (m_is_program_k7_0) {
        ...
        if (str_child_dir >= 'A') {
            ...
            Log.set_last_err("conflicted version of rf2(%s) & rf3(%s)",
                             basic_sp1401::hw_ver2str(get_rf_ver(2)).c_str(),
                             basic_sp1401::hw_ver2str(get_rf_ver(3)).c_str());
        }
    }
    ...
    if (m_is_program_k7_1) {
        ...
        if (str_child_dir >= 'A') {
            ...
            Log.set_last_err("conflicted version of rf0(%s) & rf1(%s)",
                             basic_sp1401::hw_ver2str(get_rf_ver(0)).c_str(),
                             basic_sp1401::hw_ver2str(get_rf_ver(1)).c_str());
        }
    }
...
char sp3301::rf_ver2child_dir(hw_ver_t ver0, hw_ver_t ver1)
{
    ...
    if (HW_ERROR == ver0)
        decision_ver = ((HW_ERROR == ver1) ? hw_ver_t(HW_MAX - 1) : ver1);
    else
...
int32_t sp3301::rf_get_tx_freq(uint32_t rf_idx,uint64_t &freq)
{
...
int32_t sp3301::arb_load(uint32_t rf_idx, const char *path)
{
    ...
    if ((fp[0] = sp1401[0]->get_arb_reader()->load(&arb_param[0],nullptr)) == nullptr)
		return -1;
    if ((fp[1] = sp1401[1]->get_arb_reader()->load(&arb_param[1],nullptr)) == nullptr)
		return -1;
...
int32_t sp3301::rf_get_rx_freq(uint32_t rf_idx,uint64_t &freq)
{
