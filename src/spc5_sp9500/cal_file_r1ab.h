...
class RD_API cal_file_r1ab : public sp_rd::cal_file
{
    DECLARE_CAL_FILE()

public:
    cal_file_r1ab(sp1401::hw_ver_t ver,uint32_t rf_idx,uint32_t rfu_idx);
...
public:
    void get_tx_att(uint64_t freq,sp_rd::sp1401::io_mode_t mode,float *att);
    void get_tx_att(uint64_t freq,sp_rd::sp1401::io_mode_t mode,int32_t att,double &offset);
    void get_tx_pwr(uint64_t freq,double pwr,sp_rd::sp1401::io_mode_t mode,
                    double &d_gain,int32_t &att1,int32_t &att2,sp_rd::sp1401::tx_pa_att_t &pa_att,int32_t &att3);
public:
