...
class RD_API cal_file : boost::noncopyable
{
public:
...
public:
    cal_file(sp1401::hw_ver_t ver,uint32_t rf_idx,uint32_t rfu_idx);
    virtual ~cal_file();
...
public:
    sp1401::hw_ver_t hw_ver() {return m_hw_ver;}
    uint32_t rf_idx()  {return m_rf_idx; }
    uint32_t rfu_idx() {return m_rfu_idx;}
    uint32_t ordinal() {return m_ordinal;}
    sp1401::bw_t bw()  {return m_bw;}
    static x9119_table *x9119();
    static uint32_t get_max_item_byte();

    virtual void set_bw(sp_rd::sp1401::bw_t bw) {m_bw = bw;}
    void set_hw_ver(sp1401::hw_ver_t ver) {m_hw_ver = ver;}
