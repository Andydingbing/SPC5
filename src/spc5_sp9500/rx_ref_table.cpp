...
void rx_ref_table_r1ab::map2mem()
{
    ...
    for (idx_m = 0;idx_m < RF_RX_FREQ_PTS_CALLED_R1A;idx_m ++) {
        freq = RF_RX_FREQ_STAR + idx_m * RF_RX_FREQ_STEP_CALLED_R1A;
...
void rx_ref_table_r1ab::guess_base_op(int32_t &lna_att,double &att1,int32_t &att2)
{
    rx_ref_table_r1ab::rx_state_f_t rx_state_op[] = {
		/*********************************************************************/
                                            //Ref   //SG    //
        {0,int32_t(RX_ATT),0,-30,-20,0},	//30	//10	//_0dBFS_L - 20dBm
        {0,int32_t(RX_ATT),0,-20,-20,0},	//20	//10	//_0dBFS_L - 10dBm
        {0,int32_t(RX_ATT),0,-20,-10,0},	//10	//10	//_0dBFS_L
        {0,int32_t(RX_ATT),0,-15,-10,0},	//0		//0		//_0dBFS_L
        {0,int32_t(RX_ATT),0,-10, 0 ,0},	//-10	//-10	//_0dBFS_L (IL)
        {0,int32_t(RX_ATT),0,  0, 0 ,0},	//-20	//-20	//_0dBFS_L

        {0,int32_t(RX_LNA),0,  0, 0 ,0},	//-30	//-30	//_0dBFS_H
		/*********************************************************************/
        {0,int32_t(RX_ATT),0,  0, 0 ,0},	//-30			//_0dBFS_L - 10dB
        {0,int32_t(RX_ATT),0,  0, 0 ,0},	//-40			//_0dBFS_L - 20dB
        {0,int32_t(RX_ATT),0,  0, 0 ,0},	//-50			//_0dBFS_L - 30dB

        {0,int32_t(RX_LNA),0,  0, 0 ,0},	//-60			//_0dBFS_H - 30dB
        {0,int32_t(RX_LNA),0,  0, 0 ,0},	//-70			//_0dBFS_H - 40dB
        {0,int32_t(RX_LNA),0,  0, 0 ,0},	//-80			//_0dBFS_H - 50dB
        {0,int32_t(RX_LNA),0,  0, 0 ,0},	//-90			//_0dBFS_H - 60dB
        {0,int32_t(RX_LNA),0,  0, 0 ,0},	//-100			//_0dBFS_H - 70dB
	};
...
void rx_ref_table_r1ab::guess_base_io(int32_t &lna_att,double &att1,int32_t &att2)
{
    rx_ref_table_r1ab::rx_state_f_t rx_state_io[] = {
        ...
        {0,int32_t(RX_ATT),0,-20,-20,0},	//20	//10	//_0dBFS_L - 10dBm
        {0,int32_t(RX_ATT),0,-20,-10,0},	//10	//10	//_0dBFS_L
        {0,int32_t(RX_ATT),0,-10,-10,0},	//0		//0		//_0dBFS_L
        {0,int32_t(RX_ATT),0,-10, 0 ,0},	//-10	//-10	//_0dBFS_L (IL)
        {0,int32_t(RX_ATT),0, 0,  0 ,0},	//-20	//-20	//_0dBFS_L
...
void rx_ref_table_r1ab::get_io(uint64_t freq,double ref,int64_t &ad_0dbfs,int32_t &lna_att,double &att1,int32_t &att2)
{
    ...
    uint32_t freq_idx_l = SERIE_INDEX(freq_l,RF_RX_FREQ_STAR,RF_RX_FREQ_STEP_CALLED_R1A);
    uint32_t freq_idx_r = freq >= RF_RX_FREQ_STOP ? freq_idx_l : SERIE_INDEX(freq_r,RF_RX_FREQ_STAR,RF_RX_FREQ_STEP_CALLED_R1A);
...
void rx_ref_op_table_r1cd::get_base(uint64_t freq,data_m_t *data)
{
    ...
    uint32_t freq_idx_l = SERIE_INDEX(freq_l,RF_RX_FREQ_STAR,RF_RX_FREQ_STEP_CALLED_R1C);
    uint32_t freq_idx_r = freq >= RF_RX_FREQ_STOP ? freq_idx_l : SERIE_INDEX(freq_r,RF_RX_FREQ_STAR,RF_RX_FREQ_STEP_CALLED_R1C);
...
void rx_ref_op_table_r1cd::get_base(uint64_t freq,data_f_t *data)
{
    ...
    *data = m_data_f[SERIE_INDEX(freq_l,RF_RX_FREQ_STAR,RF_RX_FREQ_STEP_INFILE)];
    data->freq = freq;
...
void rx_ref_op_table_r1cd::get(hw_ver_t ver,uint64_t freq,double ref,rx_state_f_t *state,tm *time)
{
    ...
    if (time != nullptr) { *time = data.time; }
}
