...
#ifdef RD_PLATFORM_WIN32
    ::GetModuleFileNameA(nullptr,m_base_path,sizeof(m_base_path));
...
int gen_ini_file::get_config_str_value(const char *section, const char *key, char *value)
{
    ...
    if (nullptr == (fp = fopen(exact_path,"r")))
		return FILENAME_NOTEXIST;

    while (!feof(fp)) {                                     //if end of the file
        if (nullptr == fgets(buffer,150,fp))
			break;
        str1 = buffer;
...
{
    char buffer[160] = {0};
    char *str1 = nullptr,*str2 = nullptr,*str3 = nullptr;
    unsigned int len = 0;
    int ret = 0;
