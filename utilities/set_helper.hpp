...
RD_INLINE uint32_t sections(const std::string &str)
{ return set_helper_t<boost::any>::sections(str); }

RD_INLINE uint32_t sections(const char *str)
{ return set_helper_t<boost::any>::sections(str); }

template<typename T = std::string,
template<typename E = T,typename allocator_t = std::allocator<E>> class container>
RD_INLINE void split(const std::string &str,uint32_t sections,container<T> &out)
...
RD_INLINE void split(const char *str,uint32_t sections,char (*out)[32])
{ return set_helper_t<boost::any>::split(str,sections,out); }

RD_INLINE void split(const std::string &str,range_string &out)
{ return set_helper_t<boost::any>::split(str,out); }

template<typename T>
RD_INLINE uint32_t parse(const std::string &str,set_helper_t<T> &set)
{ return set.parse_from(str); }

template<typename T>
RD_INLINE uint32_t parse(const std::string &str,range_freq<T> &out)
{ return out.parse_from(str); }

template<typename T>
RD_INLINE uint32_t parse(const std::string &str,range_pwr<T> &out)
{ return out.parse_from(str); }

template<typename T>
RD_INLINE set_helper_t<T> parse(const std::string &str)
{ set_helper_t<T> set;set.parse_from(str);return set; }

template<typename T>
RD_INLINE range_freq<T> parse_to_freq(const std::string &str)
{ range_freq<T> out;out.parse_from(str);return out; }

template<typename T>
RD_INLINE range_pwr<T> parse_to_pwr(const std::string &str)
{ range_pwr<T> out; out.parse_from(str); return out; }

template<typename T>
RD_INLINE uint32_t pts(const std::string &str)
