...
{
    if (str.length() < 1) {
        RD_THROW rd::syntax_error("Invalid string : \"" + str + "\",it's empty.");
    }
    ...
    if (str_copy.find(' ') != std::string::npos) {
        RD_THROW rd::syntax_error("Invalid string : \"" + str_copy + "\",with space character.");
    }
    ...
    if (std::regex_match(str_copy,reg_normal)) {
        ...
        RD_THROW rd::syntax_error("Invalid string : \""  + str_copy + "\".");
    }
}
