    ...
    ui(new Ui::Q_SP1403_R1A_Widget)
{
    ...
    for (size_t i = 0;i < tx_sw1_t::_size();++i) {
        ui->comboBoxTX0Sw1->addItem(QString::fromUtf8(tx_sw1_t::_names()[i]));
        ui->comboBoxTX1Sw1->addItem(QString::fromUtf8(tx_sw1_t::_names()[i]));
    }

    for (size_t i = 0;i < tx_sw2_t::_size();++i) {
        ui->comboBoxTX0Sw2->addItem(QString::fromUtf8(tx_sw2_t::_names()[i]));
        ui->comboBoxTX1Sw2->addItem(QString::fromUtf8(tx_sw2_t::_names()[i]));
    }

    for (size_t i = 0;i < tx_sw3_t::_size();++i) {
        ui->comboBoxTX0Sw3->addItem(QString::fromUtf8(tx_sw3_t::_names()[i]));
        ui->comboBoxTX1Sw3->addItem(QString::fromUtf8(tx_sw3_t::_names()[i]));
    }

    for (size_t i = 0;i < tx_sw4_t::_size();++i) {
        ui->comboBoxTX0Sw4->addItem(QString::fromUtf8(tx_sw4_t::_names()[i]));
        ui->comboBoxTX1Sw4->addItem(QString::fromUtf8(tx_sw4_t::_names()[i]));
    }

    for (size_t i = 0;i < tx0_sw5_t::_size();++i) {
        ui->comboBoxTX0Sw5->addItem(QString::fromUtf8(tx0_sw5_t::_names()[i]));
    }

    for (size_t i = 0;i < tx1_sw5_t::_size();++i) {
        ui->comboBoxTX1Sw5->addItem(QString::fromUtf8(tx0_sw5_t::_names()[i]));
    }

    for (size_t i = 0;i < tx0_sw6_t::_size();++i) {
        ui->comboBoxTX0Sw6->addItem(QString::fromUtf8(tx0_sw6_t::_names()[i]));
    }

    for (size_t i = 0;i < tx1_sw6_t::_size();++i) {
        ui->comboBoxTX1Sw6->addItem(QString::fromUtf8(tx1_sw6_t::_names()[i]));
    }

    for (size_t i = 0;i < tx0_sw7_t::_size();++i) {
        ui->comboBoxTX0Sw7->addItem(QString::fromUtf8(tx0_sw7_t::_names()[i]));
    }

    for (size_t i = 0;i < tx0_sw8_t::_size();++i) {
        ui->comboBoxTX0Sw8->addItem(QString::fromUtf8(tx0_sw8_t::_names()[i]));
    }

    for (size_t i = 0;i < tx0_sw9_t::_size();++i) {
        ui->comboBoxTX0Sw9->addItem(QString::fromUtf8(tx0_sw9_t::_names()[i]));
    }

    for (size_t i = 0;i < tx0_sw10_t::_size();++i) {
        ui->comboBoxTX0Sw10->addItem(QString::fromUtf8(tx0_sw10_t::_names()[i]));
    }

    for (size_t i = 0;i < tx0_sw11_t::_size();++i) {
        ui->comboBoxTX0Sw11->addItem(QString::fromUtf8(tx0_sw11_t::_names()[i]));
    }

    for (size_t i = 0;i < tx0_sw12_t::_size();++i) {
        ui->comboBoxTX0Sw12->addItem(QString::fromUtf8(tx0_sw12_t::_names()[i]));
    }

    for (size_t i = 0;i < rx_lna_att_t::_size();++i) {
        ui->comboBoxRXLNAAttSw->addItem(QString::fromUtf8(rx_lna_att_t::_names()[i]));
    }
