#include "qtestr1crxfreqresponsetabwidget.h"
#include <QHBoxLayout>
...
// QTestR1CRxFreqResponseWidget

QTestR1CRxFreqResponseWidget::QTestR1CRxFreqResponseWidget(QWidget *parent) : QWidget(parent)
{
    model = NULL;

    plot = new QCalPlot(this);
    plot->init(RF_TX_FREQ_STAR/1000000.0,RF_TX_FREQ_STOP/1000000.0,-80.0,-10.0);
...
void QTestR1CRxFreqResponseWidget::init(QR1CRxFreqResponseModel *model)
{
    this->model = model;
//    tableView->setModel(model);
}

void QTestR1CRxFreqResponseWidget::resetShowWidget(qint32 pts, qint32 sec)
{
    ...
    for (qint32 i = 0;i < sec;i ++) {
        model->iterTable()->append(new QwtRxFreqResponseData);
...
// QCalR1CRxFreqResponseTabWidget
QTestR1CRxFreqResponseTabWidget::QTestR1CRxFreqResponseTabWidget(QWidget *parent) :
    QTabWidget(parent)
{
    QWidget *widget_80 = new QWidget;
    widgetOP_80 = new QTestR1CRxFreqResponseWidget(widget_80);
    widgetIO_80 = new QTestR1CRxFreqResponseWidget(widget_80);
    QwtText textTitle = widgetOP_80->plot->title();
    textTitle.setText(tr("RxRfFreqResponse Curve(X:Freq(MHz) Y:Pow(dB))"));
    widgetOP_80->plot->setTitle(textTitle);
    textTitle.setText(tr("RxIfFreqResponse Curve(X:Freq(MHz) Y:Pow(dB))"));
    widgetIO_80->plot->setTitle(textTitle);
    widgetIO_80->plot->init(IF_RX_FILTER_160M_FREQ_STAR/1000000.0,IF_RX_FILTER_160M_FREQ_STOP/1000000.0,-75.0,-5.0);
...
void QTestR1CRxFreqResponseTabWidget::init(QR1CRxFreqResponseModel *modelOP,QR1CRxFreqResponseModel *modelIO)
{
    widgetOP_80->init(modelOP);
    widgetIO_80->init(modelIO);
}

void QTestR1CRxFreqResponseTabWidget::resetShowWidget(TestParam *param)
{
    int64_t freqStar = 0;
    int64_t freqStop = 0;
