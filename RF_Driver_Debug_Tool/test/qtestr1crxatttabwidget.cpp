#include "qtestr1crxatttabwidget.h"
#include <QHBoxLayout>
...
QTestR1CRxAttTabWidget::QTestR1CRxAttTabWidget(QWidget *parent) :
    QTabWidget(parent)
{
    _modelOP = nullptr;
    _modelIO = nullptr;
    ...
    plotOP->init(30,-40,-1,1);
    plotIO->init(30,-40,-1,1);
...
void QTestR1CRxAttTabWidget::init(QR1CRxAttTestOPModel *modelOP,QR1CRxAttTestIOModel *modelIO)
{
     _modelOP = modelOP;
     _modelIO = modelIO;
...
void QTestR1CRxAttTabWidget::resetShowWidget(TestParam *param)
{
    qint64 freqStar = 0;
    qint64 freqStop = 0;
    qint32 pts = freq2array(param->rfFreqStar,param->rfFreqStop,param->rfFreqStep,nullptr,nullptr,nullptr,&freqStop,&freqStar);
    ...
    if (testOP(param->mode)) {
        ...
        for (qint32 i = 0;i <100;i ++) {
            ...
            for (qint32 j = 0;j < sec;j ++) {
                _modelOP->iterTable(i)->append(new QwtRxAttTestOPData(i));
    ...
    if (testIO(param->mode)) {
        ...
        for (qint32 i = 0;i < 100;i ++) {
            ...
            for (qint32 j = 0;j < sec;j ++) {
                _modelIO->iterTable(i)->append(new QwtRxAttTestIOData(i));
