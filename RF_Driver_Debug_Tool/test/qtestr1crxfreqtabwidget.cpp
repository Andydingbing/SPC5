#include "test/qtestr1crxfreqtabwidget.h"
#include <QHBoxLayout>
...
// QTestR1CRxFreqWidget

QTestR1CRxFreqWidget::QTestR1CRxFreqWidget(QWidget *parent) : QWidget(parent)
{
    model = NULL;

    plot = new QCalPlot(this);
    plot->init(RF_TX_FREQ_STAR/1000000.0,RF_TX_FREQ_STOP/1000000.0,-1.5,1.5);
...
void QTestR1CRxFreqWidget::init(QR1CRxFreqModel *model)
{
    this->model = model;
//    tableView->setModel(model);
}

void QTestR1CRxFreqWidget::resetShowWidget(qint32 pts, qint32 sec)
{
    ...
    for (qint32 i = 0;i < sec;i ++) {
        model->iterTable()->append(new QwtRxFreqTestData);
...
QTestR1CRxFreqTabWidget::QTestR1CRxFreqTabWidget(QWidget *parent) :
    QTabWidget(parent)
{
    QWidget *widget_80 = new QWidget;
    widgetOP_80 = new QTestR1CRxFreqWidget(widget_80);
    widgetIO_80 = new QTestR1CRxFreqWidget(widget_80);
    QwtText textTitle = widgetOP_80->plot->title();
    textTitle.setText(tr("RxRfFreq Curve(X:Freq(MHz) Y:Pow(dB))"));
    widgetOP_80->plot->setTitle(textTitle);
    textTitle.setText(tr("RxIfFreq Curve(X:Freq(MHz) Y:Pow(dB))"));
    widgetIO_80->plot->setTitle(textTitle);
    widgetIO_80->plot->init(IF_RX_FILTER_160M_FREQ_STAR/1000000.0,IF_RX_FILTER_160M_FREQ_STOP/1000000.0,-1.0,1.0);
...
void QTestR1CRxFreqTabWidget::init(QR1CRxFreqModel *modelOP,QR1CRxFreqModel *modelIO)
{
    widgetOP_80->init(modelOP);
    widgetIO_80->init(modelIO);
}

void QTestR1CRxFreqTabWidget::resetShowWidget(TestParam *param)
{
    int64_t freqStar = 0;
    int64_t freqStop = 0;
