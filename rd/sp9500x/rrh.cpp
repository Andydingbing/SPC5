...
{
    SP9500PRO_RRH_REG_DECL(0x07);

    INT_CHECK(get_reg(0x07,SP9500PRO_RRH_REG_DATA(0x07)));
    SP9500PRO_RRH_REG(0x07).state = en ? 0 : 1;
    return set_reg(0x07,SP9500PRO_RRH_REG_DATA(0x07));
}
...
int32_t rrh::get_pwr_en(bool &en) const
{
    ...
    INT_CHECK(get_reg(0x07,SP9500PRO_RRH_REG_DATA(0x07)));
    en = SP9500PRO_RRH_REG(0x07).state == 0 ? true : false;
    return 0;
}
...
int32_t rrh::set_io_mode(const ns_rrh::port_t port,const io_mode_t mode) const
{
    ...
    if (port == ns_rrh::TRX0) {
        switch (mode) {
        case IO :     { SP9500PRO_RRH_REG(0x07).sw = 0; break; }
        case INPUT :  { SP9500PRO_RRH_REG(0x07).sw = 1; break; }
        case OUTPUT : { SP9500PRO_RRH_REG(0x07).sw = 2; break; }
        case CLOSE :  { SP9500PRO_RRH_REG(0x07).sw = 3; break; }
        default:      { return -1; }
        }
    } else {
        switch (mode) {
        case IO :     { SP9500PRO_RRH_REG(0x07).sw = 3; break; }
        case INPUT :  { SP9500PRO_RRH_REG(0x07).sw = 2; break; }
        case OUTPUT : { SP9500PRO_RRH_REG(0x07).sw = 1; break; }
        case CLOSE :  { SP9500PRO_RRH_REG(0x07).sw = 0; break; }
        default:      { return -1; }
        }
    }

    INT_CHECK(set_reg(0x07,SP9500PRO_RRH_REG_DATA(0x07)));
    return 0;
}
...
int32_t rrh::set_io_mode(const io_mode_t mode_0,const io_mode_t mode_1) const
{
    ...
    if (mode_0 == IO && mode_1 == CLOSE) {
        SP9500PRO_RRH_REG(0x07).sw = 0;
    } else if (mode_0 == INPUT && mode_1 == OUTPUT) {
        SP9500PRO_RRH_REG(0x07).sw = 1;
    } else if (mode_0 == OUTPUT && mode_1 == INPUT) {
        SP9500PRO_RRH_REG(0x07).sw = 2;
    } else if (mode_0 == CLOSE && mode_1 == IO) {
        SP9500PRO_RRH_REG(0x07).sw = 3;
    } else {
        return -1;
    }
...
int32_t rrh::get_io_mode(const ns_rrh::port_t port,io_mode_t &mode) const
{
    ...
    switch (SP9500PRO_RRH_REG(0x07).sw) {
...
{
    SP9500PRO_RRH_REG_DECL(0x03);

    INT_CHECK(get_reg(0x03,SP9500PRO_RRH_REG_DATA(0x03)));
    SP9500PRO_RRH_REG(0x03).tx_pwr_sign = pwr > 0 ? 0 : 1;
    SP9500PRO_RRH_REG(0x03).tx_pwr = unsigned(abs(int16_t(pwr * 10.0)));
    INT_CHECK(set_reg(0x03,SP9500PRO_RRH_REG_DATA(0x03)));
    return 0;
}
...
int32_t rrh::get_tx_pwr(double &pwr) const
{
    ...
    INT_CHECK(get_reg(0x03,SP9500PRO_RRH_REG_DATA(0x03)));
    pwr  = SP9500PRO_RRH_REG(0x03).tx_pwr / 10.0;
    pwr *= SP9500PRO_RRH_REG(0x03).tx_pwr_sign ? -1.0 : 0.0;
    return 0;
}
...
int32_t rrh::set_rx_ref(const double pwr) const
{
    ...
    INT_CHECK(get_reg(0x03,SP9500PRO_RRH_REG_DATA(0x03)));
    SP9500PRO_RRH_REG(0x03).rx_ref_sign = pwr > 0 ? 0 : 1;
    SP9500PRO_RRH_REG(0x03).rx_ref = unsigned(abs(int16_t(pwr * 10.0)));
    INT_CHECK(set_reg(0x03,SP9500PRO_RRH_REG_DATA(0x03)));
    return 0;
}
...
int32_t rrh::get_rx_ref(double &pwr) const
{
    ...
    INT_CHECK(get_reg(0x03,SP9500PRO_RRH_REG_DATA(0x03)));
    pwr  = SP9500PRO_RRH_REG(0x03).rx_ref / 10.0;
    pwr *= SP9500PRO_RRH_REG(0x03).rx_ref_sign ? -1.0 : 0.0;
    return 0;
}
...
int32_t rrh::set_if_loop_sw(const bool en) const
{
    ...
    INT_CHECK(get_reg(0x07,SP9500PRO_RRH_REG_DATA(0x07)));
    SP9500PRO_RRH_REG(0x07).if_loop = en ? 1 : 0;
    return set_reg(0x07,SP9500PRO_RRH_REG_DATA(0x07));
}
...
int32_t rrh::get_if_loop_sw(bool &en) const
{
    ...
    INT_CHECK(get_reg(0x07,SP9500PRO_RRH_REG_DATA(0x07)));
    en = SP9500PRO_RRH_REG(0x07).if_loop ? true : false;
    return 0;
}
...
int32_t rrh::get_det(uint16_t &det) const
{
    ...
    INT_CHECK(get_reg(0x07,SP9500PRO_RRH_REG_DATA(0x07)));
    det = SP9500PRO_RRH_REG(0x07).det;
    return 0;
}

int32_t rrh::set_reg(const uint8_t addr,const uint32_t data) const
{
    SP9500PRO_RFU_V9_REG_DECL_2(0x0105,0x0125);
    SP9500PRO_RFU_V9_REG_DECL_2(0x0106,0x0126);

    SP9500PRO_RFU_V9_REG_2(0x0105,0x0125).addr = addr;
    SP9500PRO_RFU_V9_REG_2(0x0105,0x0125).wr = 0;
    SP9500PRO_RFU_V9_REG_2(0x0106,0x0126).data = data;
    SP9500PRO_RFU_V9_W_2(0x0106,0x0126);
    SP9500PRO_RFU_V9_OP_2(0x0105,0x0125);
    SP9500PRO_RFU_V9_WAIT_IDLE_2(0x0105,0x0125,0,INT_MAX);
    return 0;
}
...
{
    SP9500PRO_RFU_V9_REG_DECL_2(0x0105,0x0125);
    SP9500PRO_RFU_V9_REG_DECL_2(0x0107,0x0127);

    SP9500PRO_RFU_V9_REG_2(0x0105,0x0125).addr = addr;
    SP9500PRO_RFU_V9_REG_2(0x0105,0x0125).wr = 1;
    SP9500PRO_RFU_V9_OP_2(0x0105,0x0125);
    SP9500PRO_RFU_V9_WAIT_IDLE_2(0x0105,0x0125,0,10000);
    SP9500PRO_RFU_V9_R_2(0x0107,0x0127);
    data = SP9500PRO_RFU_V9_REG_2(0x0107,0x0127).data;
    return 0;
}
