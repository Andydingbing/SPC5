...
void arb_reader::get_file_info(string &info)
{
    ...
    info += (format("File Size = %d\r\n") % param.size).str();
    info += "********** File Header **********\r\n";
    info += (format("File Name = %s\r\n") % param.header.file_name).str();
    info += (format("Date = %s\r\n")      % param.header.date).str();
    info += (format("Version = %s\r\n")   % param.header.version).str();
    info += (format("Segments = %s\r\n")  % param.header.segments).str();
    info += (format("Checksum = %0X\r\n") % param.header.checksum).str();
    info += "*********************************\r\n";
    ...
    for (iter_seg_header = param.seg_header.begin();iter_seg_header != param.seg_header.end();++iter_seg_header) {
        ...
        info += (format("Segment:%d\r\n")         % seg_idx).str();
        info += (format(" Samples = %d\r\n")      % iter_seg_header->samples).str();
        info += (format(" Duration = %d\r\n")     % iter_seg_header->duration).str();
        info += (format(" Level Offset = %f\r\n") % iter_seg_header->level_offset).str();
        info += (format(" Peak  Offset = %f\r\n") % iter_seg_header->peak_offset).str();
        info += (format(" Author = %s\r\n")       % iter_seg_header->auth_info).str();
        info += (format(" Name = %s\r\n")         % iter_seg_header->name).str();
        info += (format(" Marker Bytes = %d\r\n") % iter_seg_header->marker_bytes).str();
        info += (format(" Makers = %d\r\n")       % num_markers).str();
        info += (format(" SR = %f\r\n")           % iter_seg_header->sr).str();

        marker_list::const_iterator iter;

        for (iter = iter_marker_list->begin();iter != iter_marker_list->end();iter ++) {
            info += (format("   Marker %d:\r\n") % iter->idx).str();
            info += (format("    Type = %s\r\n") % marker_type_t::_from_integral(iter->type)._to_string()).str();
            if (iter->type == marker_type_t::Period) {
                info += (format("    Start    = %d\r\n") % iter->marker.period.start).str();
                info += (format("    Duration = %d\r\n") % iter->marker.period.duration).str();
                info += (format("    Period   = %d\r\n") % iter->marker.period.period).str();
            } else if (iter->type == marker_type_t::UnPeriod) {
                for (uint32_t i = 0;i < iter->marker.unperiod.cnt;++i) {
                    info += (format("    Start    = %d\r\n") % iter->marker.unperiod.start[i]).str();
                    info += (format("    Duration = %d\r\n") % iter->marker.unperiod.duration[i]).str();
				}
            } else {
