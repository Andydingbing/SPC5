...
public:
    ...
    RD_INLINE size_t samples() const { return _i.size(); }

    double rbw() const { return double(_sr) / double(samples()); }

    void set_full_scale(double full_scale) { _full_scale = full_scale; }
    RD_INLINE double full_scale() const { return _full_scale; }

    void set_sr(uint64_t sr);
    RD_INLINE uint64_t sr() const { return _sr; }
    ...
    double  magnitude(const uint32_t idx) const { return _magnitude.at(idx); }
