...
IMPLEMENT_DYNAMIC(CCalR1CTxAttTabCtrl, CCSETabCtrl)

CCalR1CTxAttTabCtrl::CCalR1CTxAttTabCtrl()
{
	m_pChartOP = NULL;
	m_pChartIO = NULL;
	m_pListOP = NULL;
...
BEGIN_MESSAGE_MAP(CCalR1CTxAttTabCtrl, CCSETabCtrl)
	ON_WM_SIZE()
	ON_NOTIFY_REFLECT(TCN_SELCHANGE, &CCalR1CTxAttTabCtrl::OnTcnSelchange)
END_MESSAGE_MAP()


void CCalR1CTxAttTabCtrl::AttachChild()
{
	...
	m_pListOP->Create(WS_CHILD | WS_VISIBLE | WS_TABSTOP | LVS_OWNERDATA | LVS_ALIGNLEFT | LVS_REPORT,crCtrlL,this,IDC_LIST_TX_ATT_R1C_OP);
	...
	m_pListIO->Create(WS_CHILD | WS_VISIBLE | WS_TABSTOP | LVS_OWNERDATA | LVS_ALIGNLEFT | LVS_REPORT,crCtrlR,this,IDC_LIST_TX_ATT_R1C_IO);
...
void CCalR1CTxAttTabCtrl::ResetShowCtrl(CalIOMode Mode)
{
	if (CalOP(Mode)) {
		m_DataOP.clear();
		...
		m_pListOP->InsertColumn(1,_T("Power"),LVCFMT_RIGHT,60);
		m_pListOP->InsertColumn(2,_T("Att0"),LVCFMT_RIGHT,40);
		m_pListOP->InsertColumn(3,_T("Att1"),LVCFMT_RIGHT,40);
		m_pListOP->InsertColumn(4,_T("Att2"),LVCFMT_RIGHT,40);
		m_pListOP->InsertColumn(5,_T("Att3"),LVCFMT_RIGHT,40);
		m_pListOP->InsertColumn(6,_T("DGain"),LVCFMT_RIGHT,70);
		m_pListOP->InsertColumn(7,_T("Offset"),LVCFMT_RIGHT,70);
		m_pListOP->InsertColumn(8,_T("Temp4_TxLO2"),LVCFMT_CENTER,100);
		m_pListOP->InsertColumn(9,_T("Temp5_TxLO1"),LVCFMT_CENTER,100);
		m_pListOP->InsertColumn(10,_T("Temp6_TxPA4"),LVCFMT_CENTER,100);
		m_pListOP->InsertColumn(11,_T("Temp7_TxPA3"),LVCFMT_CENTER,100);
		m_pListOP->InsertColumn(12,_T("End Time"),LVCFMT_RIGHT,140);
	...
	if (CalIO(Mode)) {
		...
		m_pListIO->InsertColumn(1,_T("Power"),LVCFMT_RIGHT,60);
		m_pListIO->InsertColumn(2,_T("Att0"),LVCFMT_RIGHT,40);
		m_pListIO->InsertColumn(3,_T("Att1"),LVCFMT_RIGHT,40);
		m_pListIO->InsertColumn(4,_T("Att2"),LVCFMT_RIGHT,40);
		m_pListIO->InsertColumn(5,_T("Att3"),LVCFMT_RIGHT,40);
		m_pListIO->InsertColumn(6,_T("DGain"),LVCFMT_RIGHT,70);
		m_pListIO->InsertColumn(7,_T("Offset"),LVCFMT_RIGHT,70);
		m_pListIO->InsertColumn(8,_T("Temp4_TxLO2"),LVCFMT_CENTER,100);
		m_pListIO->InsertColumn(9,_T("Temp5_TxLO1"),LVCFMT_CENTER,100);
		m_pListIO->InsertColumn(10,_T("Temp6_TxPA4"),LVCFMT_CENTER,100);
		m_pListIO->InsertColumn(11,_T("Temp7_TxPA3"),LVCFMT_CENTER,100);
		m_pListIO->InsertColumn(12,_T("End Time"),LVCFMT_RIGHT,140);
...
void CCalR1CTxAttTabCtrl::ClearLineSerieOP()
{
...
void CCalR1CTxAttTabCtrl::ClearLineSerieIO()
{
...
void CCalR1CTxAttTabCtrl::OnSize(UINT nType, int cx, int cy)
{
...
void CCalR1CTxAttTabCtrl::OnTcnSelchange(NMHDR *pNMHDR, LRESULT *pResult)
{
