...
typedef int32_t (CTestR1ATxFRThread::*pBeginTestFR)();

IMPLEMENT_DYNCREATE(CTestR1ATxFRThread, CCSEWinThread)

CTestR1ATxFRThread::CTestR1ATxFRThread()
{
}

CTestR1ATxFRThread::CTestR1ATxFRThread(CWnd *pParent,TestRFFRInfo *pInfo)
{
	m_pParent = pParent;
...
CTestR1ATxFRThread::CTestR1ATxFRThread(CWnd *pParent,TestBBFRInfo *pInfo)
{
	m_pParent = pParent;
...
BOOL CTestR1ATxFRThread::InitInstance()
{
	return TRUE;
}

int CTestR1ATxFRThread::ExitInstance()
{
	return CWinThread::ExitInstance();
}


BEGIN_MESSAGE_MAP(CTestR1ATxFRThread, CWinThread)
END_MESSAGE_MAP()


int CTestR1ATxFRThread::Run()
{
	pBeginTestFR BeginTestFR[] = {&CTestR1ATxFRThread::TestRFFR,
								  &CTestR1ATxFRThread::TestBBFR};
	(this->*BeginTestFR[m_iFuncIdx])();
	return 0;
}

int32_t CTestR1ATxFRThread::TestRFFR()
{
	try
	{
		Instrument.Init();
		...
		InitSinglePos("Testing Tx RF Frequency Response",iPts);
...
int32_t CTestR1ATxFRThread::TestBBFR()
{
	try
	{
		Instrument.Init();
...
//		Instrument.SA_SetVBW(0,true);
		...
		CSP1401R1A *pSP1401 = m_TestBBFRInfo.m_pSP1401;
		...
		InitSinglePos("Testing Tx Baseband Filter Frequency Response",(int)SERIE_SIZE(MAX_BW / 2 * -1,MAX_BW / 2,iFreqStep));
