...
IMPLEMENT_DYNAMIC(CLogProgressCtrl, CProgressCtrl)

CLogProgressCtrl::CLogProgressCtrl() : m_clrTextColor(RGB(0,0,0))
									 , m_clrTextBkColor(BLACK)
									 , m_clrBarColor(RGB(40,180,45))
...
CLogProgressCtrl::~CLogProgressCtrl()
{
...
BEGIN_MESSAGE_MAP(CLogProgressCtrl, CProgressCtrl)
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
END_MESSAGE_MAP()


BOOL CLogProgressCtrl::OnEraseBkgnd(CDC* pDC)
{
	return TRUE;
}

void CLogProgressCtrl::OnPaint()
{
	...
	if (m_bShowPercent)
	{
		...
		strText.AppendFormat(_T("%.*f%%"),dPercent != 1,dPercent * 100.0);
	}
