...
dma_mgr::dma_mgr(vi_pci_dev *k7) : m_k7(k7) 
{
...
int32_t dma_mgr::fpga_w()
{
    ...
    RFU_K7_REG_DECLARE(0x0027);
	RFU_K7_REG_DECLARE(0x1670);

    RFU_K7_WAIT_IDLE_BUSY2(0x0027,0,INT32_MAX);
    ...
    RFU_K7_WAIT_IDLE(0x0027,0,INT32_MAX);

	INT_CHECK(fpga_w_abort());
	INT_CHECK(reset());
	INT_CHECK(fpga_w_start());

	uint32_t cur_samples[5] = { 0 };
	uint32_t samples = 0;
	...
	while (1) {
        ...
        is_timeout = time_us > 6000000  ? true : false;
//		INT_CHECK(is_iq_cap_timeout(timeout));

		for (int32_t i = 0; i < 5; i++) {
			RFU_K7_R(0x0014);
			cur_samples[i] = RFU_K7_REG(0x0014).samples;
		}
		if (!is_timeout) {
			if (cur_samples[4] == (samples * 2))	//success
				break;
			if (cur_samples[4] == cur_samples[3] && cur_samples[4] < (samples * 2)) {
				Log.set_last_err("stop at samples:%d(need samples:%d)", cur_samples[4], samples * 2);
				return -1;
			}
			if (cur_samples[4] == cur_samples[3] && cur_samples[4] > (samples * 2)) {
				Log.set_last_err("exceed,samples:%d(need samples%d)", cur_samples[4], samples * 2);
				return -1;
			}
		}
		else if (is_timeout) {
			if (cur_samples[0] < cur_samples[1])
				continue;
			if (cur_samples[4] == (samples * 2))	//also success
				break;
			if (cur_samples[4] == cur_samples[3] && cur_samples[4] < (samples * 2)) {
				Log.set_last_err("stop at samples:%d(need samples:%d)", cur_samples[4], samples * 2);
				return -1;
...
int32_t dma_mgr::set_fpga_w_samples(uint32_t samples)
{
	...
	RFU_K7_REG(0x10c6).samples = samples * 2;
	RFU_K7_W(0x10c6);
	return 0;
}
...
int32_t dma_mgr::iq2buf(uint32_t rf_idx,int16_t *I,int16_t *Q,uint32_t samples,int16_t *I_2,int16_t *Q_2)
{
    ...
    for (int i = 0;i < total_blocks;i ++) {
        ...
        for (int j = 0;j < samples_reading;j += 2) {
            ...
            iq_data_2 = ((w_package_t *)(&m_usr_spc[j]))->data[package_idx_inter_ch];
            if (I_2)
                I_2[data_idx] = iq_data_2.i;
            if (Q_2)
                Q_2[data_idx] = iq_data_2.q;
		}
        samples_left -= samples_reading;
	}
	return 0;
...
int32_t dma_mgr::r_alloc()
{
	...
	Log.add_msg_list("dmar physical address:%#x", m_dmar->phy_addr());
