...
Fs = 245.76e6;
F_intvl = 2e6;
F_star = 160e6 / 2 + 1.5 * F_intvl;         %83MHz,3MHz是为了减轻吉布斯效应
F_stop = F_star + round((1 + 1) / N * Fs);  %Bt>=2pi / N * (m + 1),过渡带暂时取m=1个点

F = [-F_stop,-F_star : F_intvl : F_star,F_stop] / (Fs / 2);
F = [-1,F,1];

fid_real = fopen('c:\\CSECal\\tx_filter_160_coef_real.txt','w');
fid_imag = fopen('c:\\CSECal\\tx_filter_160_coef_imag.txt','w');

fid = fopen('c:\\CSECal\\tx_filter_160.txt','r');
tline = fgetl(fid);
while ischar(tline)
    Rsp = str2num(tline);
    ...
    A = [0,0,1./Rsp,0,0];
    W = [0.01,1.0 * ones(1,length(Rsp)/2),0.01];
    h = cfirpm(N-1,F,A,W);
    %
