...
CChartPointsArray<T>::CChartPointsArray(unsigned iResize) 
  : m_pPoints(NULL), m_iMaxPoints(iResize), m_iCurrentPoints(0),
    m_iResize(iResize), m_Ordering(poXOrdering),m_Holding(phNoHolding)
{
...
template<class T>
void CChartPointsArray<T>::SetPoints(T* pPoints, unsigned uCount)
{
	if (uCount > m_iMaxPoints)
	{
		...
		if (phMaxHolding == m_Holding) {
			for (unsigned i=0;i < uCount;i ++)
			{
				m_pPoints[i] = m_pPoints[i] - 10000.0;
			}
		}
		else if (phMinHolding == m_Holding) {
			for (unsigned i=0;i < uCount;i ++)
			{
				m_pPoints[i] = m_pPoints[i] + 10000.0;
			}
...
// 	for (unsigned i=0;i<uCount;i++)	//custom here
// 	{
...
// 	}
	...
	{
		if (phMaxHolding == m_Holding)
			m_pPoints[i] = m_pPoints[i] >= pPoints[i] ? m_pPoints[i] : pPoints[i];
		else if (phMinHolding == m_Holding)
			m_pPoints[i] = m_pPoints[i] <= pPoints[i] ? m_pPoints[i] : pPoints[i];
		else
			m_pPoints[i] = pPoints[i];
...
void CChartPointsArray<T>::Clear()
{
	...
	if (phMaxHolding == m_Holding) {
		for (unsigned i=0;i < m_iResize;i ++)
		{
			m_pPoints[i] = m_pPoints[i] - 10000.0;
		}
	}
...
template <class T>
void CChartPointsArray<T>::SetHolding(PointsHolding newHolding)
{
	m_Holding = newHolding;
	if (phMaxHolding == newHolding) {
		for (unsigned i=0;i < m_iCurrentPoints;i ++)
		{
			m_pPoints[i] = m_pPoints[i] - 10000.0;
		}
	}
	else if (phMinHolding == newHolding) {
		for (unsigned i=0;i < m_iCurrentPoints;i ++)
		{
			m_pPoints[i] = m_pPoints[i] + 10000.0;
		}
