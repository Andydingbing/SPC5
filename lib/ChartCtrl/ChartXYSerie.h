...
struct SChartXYPoint
{
	...
	bool operator >  (const SChartXYPoint Pt) { if (X == Pt.GetX()) return Y >  Pt.GetY(); else return false;}
	bool operator >= (const SChartXYPoint Pt) { if (X == Pt.GetX()) return Y >= Pt.GetY(); else return false;}
	bool operator <	 (const SChartXYPoint Pt) { if (X == Pt.GetX()) return Y <  Pt.GetY(); else return false;}
	bool operator <= (const SChartXYPoint Pt) { if (X == Pt.GetX()) return Y <= Pt.GetY(); else return false;}
	SChartXYPoint operator - (double MinuEnd) {SChartXYPoint Pt;Pt.X = this->GetX();Pt.Y = this->GetY();Pt.Y -= MinuEnd;return Pt;}
	SChartXYPoint operator + (double AddEnd)  {SChartXYPoint Pt;Pt.X = this->GetX();Pt.Y = this->GetY();Pt.Y += AddEnd;	return Pt;}
...
public:
	...
	**/
	...
	void AddPoints(double* pX, int16_t* pY, unsigned Count);
	void AddPoints(double* pX, int32_t* pY, unsigned Count);
	...
	**/
	...
	void SetPoints(double* pX, int16_t* pY, unsigned Count);
	void SetPoints(double* pX, int32_t* pY, unsigned Count);
