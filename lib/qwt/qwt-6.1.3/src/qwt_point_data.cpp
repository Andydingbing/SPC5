...
QwtPointArrayData::QwtPointArrayData(const int16_t *x, const int16_t *y, size_t size)
{
    d_x.resize( size );
    d_y.resize( size );
    for (size_t i = 0;i < size;i ++) {
        d_x.replace(i,double(x[i]));
        d_y.replace(i,double(y[i]));
    }
}

QwtPointArrayData::QwtPointArrayData(const int16_t *y, size_t size)
{
    d_x.resize( size );
    d_y.resize( size );
    for (size_t i = 0;i < size;i ++) {
        d_x.replace(i,double(i));
        d_y.replace(i,double(y[i]));
    }
}
/*!
