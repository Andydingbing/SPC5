...
int32_t str2freq(const char *str,double &freq)
{
    if (strstr(str,"e") != NULL || strstr(str,"E") != NULL) {
        sscanf(str,"%le",&freq);
        return 0;
    }
    ...
    strcpy(copy_str,str);
    char *unit = copy_str;
    unit = unit + strlen(str) - 1;

    if (!strcmp(unit,"K") || !(strcmp(unit,"k"))) {
        unit = unit + 1 - strlen(str);
        unit[strlen(str) - 1] = '\0';
        freq = atof(unit) * 1e3;
        return 0;
    }
...
int32_t str2freq(const char *str,float &freq)
{
    double p = 0.0;
    str2freq(str,p);
    freq = (float)p;
...
int32_t str2freq(const char *str,uint64_t &freq)
{
    double p = 0.0;
    str2freq(str,p);
    freq = (uint64_t)p;
...
int32_t str2freq(const char *str,int64_t &freq)
{
    double p = 0.0;
    str2freq(str,p);
    freq = int64_t(p);
...
int32_t freq2array(const char *in1,const char *in2,int64_t step,int64_t *out,int64_t *freq_star,int64_t *freq_stop,int64_t *max,int64_t *min)
{
    int32_t no1 = str_sections(in1);
    int32_t no2 = str_sections(in2);
    if (no1 != no2)
        return -1;
    int index = 0;
    bool cal_max_min = !(NULL == max || NULL == min);
    if (cal_max_min) {
        *max = 0;
        *min = LLONG_MAX;
    }
    ...
    split_freq_character(in1,freqstar,no1);
    split_freq_character(in2,freqstop,no2);
    for (int i = 0;i < no1;i ++) {
        str2freq(freqstar[i],ifreqstar);
        str2freq(freqstop[i],ifreqstop);
        if (freq_star)
            freq_star[i] = ifreqstar;
        if (freq_stop)
            freq_stop[i] = ifreqstop;
        for (int j = 0;j < SERIE_SIZE(ifreqstar,ifreqstop,step);j ++) {
            ifreq = ifreqstar + j * step;
            if (out)
                out[index] = ifreq;
            if (cal_max_min) {
                *max = ifreq > *max ? ifreq : *max;
                *min = ifreq < *min ? ifreq : *min;
            }
            index ++;
        }
    }
...
int32_t freq_section(char *pin1,char *pin2,int64_t freq)
{
    int32_t no1 = str_sections(pin1);
    int32_t no2 = str_sections(pin2);
    if (no1 != no2)
        return -1;
    int64_t ifreqstar = 0;
    int64_t ifreqstop = 0;
