...
{
    return int(Log.get_msg_log()->size());
}
...
QVariant QMsgLogModel::data(const QModelIndex &index, int role) const
{
    if (role == Qt::DisplayRole) {
        if (0 == index.column())
            return QString::fromStdString(Log.get_msg_log()->at(index.row()).time);
        else if (1 == index.column())
            return QString::fromStdString(Log.get_msg_log()->at(index.row()).msg);
        else if (2 == index.column())
            return Log.get_msg_log()->at(index.row()).result ? "x" : "-";
        else if (3 == index.column())
            return QString("0x") +
                   QString("%1").arg(Log.get_msg_log()->at(index.row()).result,16,16,QLatin1Char('0'))
                                .toUpper()
                                .right(8);
    }
    if (role == Qt::BackgroundColorRole) {
        if (Log.get_msg_log()->at(index.row()).result)
            return CLR_ERROR;
        else
