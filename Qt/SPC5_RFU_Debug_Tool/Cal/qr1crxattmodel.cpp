...
QR1CRxAttOPModel::QR1CRxAttOPModel(sp1401_r1c *sp1401, QObject *parent)
    : QCalBaseModel(parent)
    , _sp1401(sp1401)
{
    init ();
...
void QR1CRxAttOPModel::init()
{
    ...
    for (qint8 i = 0;i < (R1C_RX_REF_OP_PTS - 3 + 1);i ++)
        iter[i].clear();
}
...
QVariant QR1CRxAttOPModel::data(const QModelIndex &index, int role) const
{
    if (Qt::DisplayRole == role) {
        ...
        if (0 == index.column())
            ...
            _sp1401->get_cal_file()->m_rx_ref_op->get(table->at(freqIdx).freq,double(ref),&dataRefOP);
...
void QR1CRxAttOPModel::setSP1401(sp1401_r1c *sp1401)
{
    _sp1401 = sp1401;
}
...
QR1CRxAttIOModel::QR1CRxAttIOModel(sp1401_r1c *sp1401, QObject *parent)
    : QCalBaseModel(parent)
    , _sp1401(sp1401)
{
    init();
...
void QR1CRxAttIOModel::init()
{
    ...
    for (qint8 i = 0;i < (R1C_RX_REF_IO_PTS - 3 + 1);i ++)
        iter[i].clear();
}
...
QVariant QR1CRxAttIOModel::data(const QModelIndex &index, int role) const
{
    if (Qt::DisplayRole == role) {
        ...
        if (0 == index.column())
            ...
            _sp1401->get_cal_file()->m_rx_ref_io->get(table->at(freqIdx).freq,double(ref),&dataRefIO);
...
void QR1CRxAttIOModel::setSP1401(sp1401_r1c *sp1401)
{
    _sp1401 = sp1401;
}
