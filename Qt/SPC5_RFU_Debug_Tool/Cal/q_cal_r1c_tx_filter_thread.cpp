...
void QCalR1CTXFilterThread::cal()
{
    ...
    if (calParam.justRebuildCoef == true) {
        SET_PROG_POS(RF_TX_FILTER_PTS + IF_TX_FILTER_160M_PTS);
    }
}
...
void QCalR1CTXFilterThread::initTXChain()
{
    ...
    SP1401->set_io_mode(sp1401::OUTPUT);
...
void QCalR1CTXFilterThread::generateFreqResponse()
{
    ...
    tx_rf_fr_table::data_f_t tempDataRFFr_80[IF_TX_FILTER_80M_PTS];
    // -43M~~~43M,step 2M.
    tx_if_fr_table::data_f_t tempDataIFFr_80[IF_TX_FILTER_80M_PTS];

    SP1401->cf()->set_bw(_80M);
    SP1401->cf()->map2buf(cal_file::TX_RF_FR_0);
    freqStarRF = quint64(2e9 + IF_TX_FILTER_80M_FREQ_STAR);
    freqStopRF = quint64(2e9 + IF_TX_FILTER_80M_FREQ_STOP);
    freqStepRF = quint64(IF_TX_FILTER_FREQ_STEP);
    ...
    freqStarIF = IF_TX_FILTER_80M_FREQ_STAR;
    freqStopIF = IF_TX_FILTER_80M_FREQ_STOP;
    freqStepIF = IF_TX_FILTER_FREQ_STEP;
    ...
    SP1401->cf()->map2buf(cal_file::TX_RF_FR_0);
    freqStarRF = RF_TX_FREQ_STAR;
    freqStopRF = 3100000000;
    freqStepRF = RF_TX_FREQ_STEP_CALLED;
    for (freqRF = freqStarRF;freqRF < freqStopRF;freqRF += freqStepRF) {
        freqCalled = freqRF < 100000000 ? 100000000 : freqRF;
        idx = 0;
        freqStarRF_InBw = quint64(freqCalled + IF_TX_FILTER_80M_FREQ_STAR);
        freqStopRF_InBw = quint64(freqCalled + IF_TX_FILTER_80M_FREQ_STOP);
        freqStepRF_InBw = quint64(IF_TX_FILTER_FREQ_STEP);
    ...
    freqStopRF = RF_TX_FREQ_STOP;
    freqStepRF = RF_TX_FREQ_STEP_CALLED;
    for (freqRF = freqStarRF;freqRF <= freqStopRF;freqRF += freqStepRF) {
        idx = 0;
        freqStarRF_InBw = quint64(freqRF + IF_TX_FILTER_80M_FREQ_STAR);
        freqStopRF_InBw = quint64(freqRF + IF_TX_FILTER_80M_FREQ_STOP);
        freqStepRF_InBw = quint64(IF_TX_FILTER_FREQ_STEP);
        for (quint64 freq = freqStarRF_InBw;freq <= freqStopRF_InBw;freq += freqStepRF_InBw) {
    ...
    tx_rf_fr_table::data_f_t tempDataRFFr_160[IF_TX_FILTER_160M_PTS];
    // -83M~~~83M,step 2M.
    tx_if_fr_table::data_f_t tempDataIFFr_160[IF_TX_FILTER_160M_PTS];

    idx = 0;
    SP1401->cf()->set_bw(_160M);
    SP1401->cf()->map2buf(cal_file::TX_RF_FR_0);
    freqStarRF = quint64(2000000000.0 + IF_TX_FILTER_160M_FREQ_STAR);
    freqStopRF = quint64(2000000000.0 + IF_TX_FILTER_160M_FREQ_STOP);
    freqStepRF = quint64(IF_TX_FILTER_FREQ_STEP);
    for (freqRF = freqStarRF;freqRF <= freqStopRF;freqRF += freqStepRF) {
    ...
    for (freqRF = freqStarRF;freqRF < freqStopRF;freqRF += freqStepRF) {
        freqCalled = freqRF < 100000000 ? 100000000 : freqRF;
        idx = 0;
        freqStarRF_InBw = quint64(freqCalled + IF_TX_FILTER_160M_FREQ_STAR);
        freqStopRF_InBw = quint64(freqCalled + IF_TX_FILTER_160M_FREQ_STOP);
        freqStepRF_InBw = quint64(IF_TX_FILTER_FREQ_STEP);
        for (quint64 freq = freqStarRF_InBw;freq <= freqStopRF_InBw;freq += freqStepRF_InBw) {
    ...
    for (freqRF = freqStarRF;freqRF <= freqStopRF;freqRF += freqStepRF) {
        idx = 0;
        freqStarRF_InBw = quint64(freqRF + IF_TX_FILTER_160M_FREQ_STAR);
        freqStepRF_InBw = quint64(freqRF + IF_TX_FILTER_160M_FREQ_STOP);
        freqStopRF_InBw = quint64(IF_TX_FILTER_FREQ_STEP);
...
void QCalR1CTXFilterThread::updateCalFile()
{
    ...
    for (quint64 freq = RF_TX_FREQ_STAR;freq <= RF_TX_FREQ_STOP;freq += RF_TX_FREQ_STEP_CALLED) {
        ...
        for (quint32 i = 0;i < TX_FILTER_ORDER_2I;i ++) {
...
void QExpR1CTXFilterThread::run()
{
    ...
    SP1401->cf()->map2buf(cal_file::TX_RF_FR_0);
    freqStarRF = RF_TX_FILTER_FREQ_STAR_0;
    freqStopRF = RF_TX_FILTER_FREQ_STOP_0;
    freqStepRF = RF_TX_FILTER_FREQ_STEP;
    ...
    SP1401->cf()->map2buf(cal_file::TX_RF_FR_1);
    freqStarRF = RF_TX_FILTER_FREQ_STAR_1;
    freqStopRF = RF_TX_FILTER_FREQ_STOP_1;
    freqStepRF = RF_TX_FILTER_FREQ_STEP;
    for (quint64 freq = freqStarRF;freq <= freqStopRF;freq += freqStepRF) {
    ...
    emit update(model_80->index(0,0),
                model_80->index(RF_TX_FREQ_PTS_CALLED,7),
                cal_file::TX_FILTER_80);
    emit update(model_160->index(0,0),
                model_160->index(RF_TX_FREQ_PTS_CALLED,7),
