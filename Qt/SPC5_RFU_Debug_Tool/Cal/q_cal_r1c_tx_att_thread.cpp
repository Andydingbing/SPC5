...
void QCalR1CTXAttThread::calOneSec(io_mode_t mode, double pwrStar, double pwrStop)
{
    ...
    if (mode == OUTPUT) {
        basePwr = (pwrStar <= R1C_TX_BASE_POWER_OP ? pwrStar : R1C_TX_BASE_POWER_OP);
    } else if (mode == IO) {
    ...
    for (quint32 i = 0;i < freqRangeCal.freqs.size();i ++) {
        ...
        if (mode == OUTPUT) {
            ...
            if (pwrStar == R1C_TX_BASE_POWER_OP || pwrStop == R1C_TX_BASE_POWER_OP - pwrStep) {
                offsetBase = 0.0;
            ...
            if (pwrStar == R1C_TX_BASE_POWER_IO || pwrStop == R1C_TX_BASE_POWER_IO - pwrStep) {
                offsetBase = 0.0;
            } else if (idxOffsetStar >= 1) {
