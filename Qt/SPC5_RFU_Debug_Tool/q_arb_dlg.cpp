...
void QArbDlg::on_pushButtonSelArbFile_clicked()
{
    ...
    if (!path.isEmpty()) {
        ...
        SP1401()->get_arb_reader()->load(path.toStdString().c_str());
        SP1401()->get_arb_reader()->get_file_info(fileInfo);
...
void QArbDlg::on_pushButtonSetAll_clicked()
{
    ...
    INT_CHECKV(SP1401()->set_arb_trigger(reTrigger, autoStart, trigSrc, trigDelay));
    ...
    INT_CHECKV(SP1401()->set_arb_param(addSamples, cycles, repMode));
...
{
    PTR_CHECK
    INT_CHECKV(SP1401()->arb_start());
}

void QArbDlg::on_pushButtonArbStop_clicked()
{
    PTR_CHECK
    INT_CHECKV(SP1401()->arb_stop());
}

void QArbDlg::on_pushButtonManualTrig_clicked()
{
    PTR_CHECK
    INT_CHECKV(SP1401()->set_arb_manual_trigger());
}

void QArbDlg::on_pushButtonLoad_clicked()
{
    PTR_CHECK
    INT_CHECKV(SP3301->arb_load(rfIdx,ui->textEditArbFilePath->toPlainText().toStdString().c_str()));
}

void QArbDlg::on_pushButtonQueryState_clicked()
{
    PTR_CHECK
    ui->lineEditCurSeg->setText(QString("%1").arg(SP1401()->get_arb_current_seg()));
    ui->lineEditCurSample->setText(QString("%1").arg(SP1401()->get_arb_current_sample()));
    ui->lineEditCurCycle->setText(QString("%1").arg(SP1401()->get_arb_current_cycle()));
    ui->lineEditCurTime->setText(QString("%1").arg(SP1401()->get_arb_current_timer()));
}

void QArbDlg::on_pushButtonQueryFrameSlot_clicked()
{
    PTR_CHECK
    ui->lineEditCurFrame->setText(QString("%1").arg(SP2401R1A()->get_cur_frame()));
    ui->lineEditCurSlot->setText(QString("%1").arg(SP2401R1A()->get_cur_slot()));
}

void QArbDlg::on_pushButtonFrameSlotTrigStart_clicked()
{
    PTR_CHECK
//    INT_CHECKV(SP2401R1A()->set_frame_slot_trigger_link(sp2401_r1a::DL));
//    INT_CHECKV(SP2401R1A()->set_frame_slot_trigger_start());
}
