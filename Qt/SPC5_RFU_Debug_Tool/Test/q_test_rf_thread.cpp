...
void QTestR1CRFThread::TX_IOSw()
{
    ...
    SP1401->set_tx_att(10.0,10.0,10.0,10.0);
...
void QTestR1CRFThread::TX_FilterSw_R1F()
{
    ...
    SP1401->set_tx_filter_sw(sp1401_r1f::BPF,sp1401_r1f::LPF);
    msleep(50);
...
void QTestR1CRFThread::TX_LOLD()
{
    ...
    for (quint32 i = 0;i < repeat;i ++) {
        THREAD_TEST_CANCEL
        for (freq = RF_TX_FREQ_STAR;freq < RF_TX_FREQ_STOP;freq += 10000000) {
            THREAD_TEST_CANCEL
            SP1401->tx_freq2lo(freq,freqLO1,freqLO2,freqLO3,filter1,filter2);
            SP1401->init_hmc1197();
            SP1401->set_adf5355(sp1401_r1c::TX_LO_2,freqLO2);
            SP1401->set_adf5355(sp1401_r1c::TX_LO_3,freqLO3);
            SP1401->set_tx_filter_sw(filter1,filter2);
            msleep(10);
            (SP1401->*det_r1c_lo_lock[0])(data.is_locked[0]);
            (SP1401->*det_r1c_lo_lock[1])(data.is_locked[1]);
            (SP1401->*det_r1c_lo_lock[2])(data.is_locked[2]);
...
void QTestR1CRFThread::TX_NoiseFloor()
{
    ...
    for (quint32 i = 0;i < freqRange.freqs.size();i ++) {
        ...
        for (quint32 j = 0;j < ARRAY_SIZE(dataPwr);j ++) {
            THREAD_TEST_CANCEL
            SP1401->set_tx_att(dataPwr[j].att0 / 2.0,dataPwr[j].att1 / 2.0,
