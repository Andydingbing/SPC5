...
void QTestR1CTempPwrThread::run()
{
    ...
    param = static_cast<TestTempPwrParam *>(testParam);

    if (param->ch == basic_sp1401::CH_TX) {
        Tx();
    } else {
        Rx();
    }
...
void QTestR1CTempPwrThread::Tx()
{
    ...
    list<common_chain_pwr_state_t>::iterator iter;
    ...
    while (1) {
        ...
        for (quint32 i = 0;i < FreqRange.freqs.size();i ++) {
            ...
            for (iter = states.begin();iter != states.end();iter ++) {
                ...
                SP1401->set_tx_att(iter->att[0],iter->att[1],iter->att[2],iter->att[3]);
                msleep(10);
