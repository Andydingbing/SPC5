...
int32_t for_0005_0006(cal_file *file)
{
    ...
    public:
        file_info_v0005_t() {
            item_info = new item_info_v0005_t[CAL_TOTAL_ITEMS];
        }
        ~file_info_v0005_t() {
...
// filter offset
    ...
    char path_new[64] = {0};
    char path_old[64] = {0};
    sprintf(path_old,"C:\\CSECal\\rfu%drf%d.cal",file->get_rfu_idx(),file->get_rf_idx());
    ...
    Log.add_msg_list(0,"file \"%s\" version 0x0005 ---> 0x0006",path_old);
    return 0;
}
