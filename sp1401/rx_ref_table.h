...
// rx reference table r1c/d
class RD_API rx_ref_table_r1cd
{
public:
    typedef struct RD_API rx_state_f_t {
        int16_t lna_att;
        int16_t att_019;
        float	att1;
        float	att2;
...
public:
    ...
    virtual void get(uint64_t freq,double ref,rx_state_m_t *state) = 0;
    virtual void get(uint64_t freq,double ref,rx_state_f_t *state,tm *time = NULL) = 0;
};
...
public:                                                                                 \
    ...
    static void get_config_table(int32_t ref,data_m_t data_ref,rx_state_m_t *state);    \
    static void guess_base(int32_t idx,int32_t &ref,int32_t &lna_att,int32_t &att_019,double &att1,double &att2,double &att3); \
    ...
    void get(uint64_t freq,double ref,rx_state_m_t *state);                             \
    void get(uint64_t freq,double ref,rx_state_f_t *state,tm *time = NULL);
