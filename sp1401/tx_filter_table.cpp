...
{
    uint32_t idx = (uint32_t)SERIE_INDEX(data->freq,RF_TX_FILTER_FREQ_STAR_0,RF_TX_FILTER_FREQ_STEP);
    m_data_f[idx] = *data;
}
...
{
    uint32_t idx = (uint32_t)SERIE_INDEX(data->freq,RF_TX_FILTER_FREQ_STAR_1,RF_TX_FILTER_FREQ_STEP);
    m_data_f[idx] = *data;
}
...
{
    uint32_t idx = (uint32_t)SERIE_INDEX(data->freq,IF_TX_FILTER_160M_FREQ_STAR,IF_TX_FILTER_FREQ_STEP);
    m_data_f[idx] = *data;
}

void tx_if_fr_table::get(double freq,data_f_t *data)
{
    data->pwr = m_data_f[(uint32_t)(SERIE_INDEX(freq,IF_TX_FILTER_160M_FREQ_STAR,IF_TX_FILTER_FREQ_STEP))].pwr;
    data->freq = freq;
}
...
void tx_filter_80m_table::get(uint64_t freq,data_f_t *data)
{
    ...
    uint32_t idx_l = (uint32_t)SERIE_INDEX(freq_l,RF_TX_FREQ_STAR,RF_TX_FREQ_STEP_INFILE);
    *data = m_data_f[idx_l];
    data->freq = freq;
}
