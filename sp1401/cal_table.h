...
#define DECLARE_CAL_ITEM_TABLE(class_name,pt)               \
    public:                                                 \
        ...
        virtual ~class_name() SAFE_DELETE_ARRAY(m_data_m);  \
    public:                                                 \
...
#define IMPLEMENT_TX_CAL_ITEM_TABLE(class_name)             \
    void class_name::add(data_f_t *data) {                  \
        uint32_t idx = (uint32_t)SERIE_INDEX(data->freq,RF_TX_FREQ_STAR,RF_TX_FREQ_STEP_INFILE);    \
        m_data_f[idx] = *data;                              \
    }

#define IMPLEMENT_RX_CAL_ITEM_TABLE(class_name)             \
    void class_name::add(data_f_t *data) {                  \
        uint32_t idx = (uint32_t)SERIE_INDEX(data->freq,RF_RX_FREQ_STAR,RF_RX_FREQ_STEP_INFILE);    \
        m_data_f[idx] = *data;								\
    }
...
namespace sp1401 {
    ...
    enum rx_lna_att_t { //r1a/b/c/d
        RX_ATT = 0,
        RX_LNA
    };
    enum rx_att_019_t {	//r1c/d
        RX_ATT_19 = 0,
        RX_ATT_0
    };
    enum io_mode_t {
