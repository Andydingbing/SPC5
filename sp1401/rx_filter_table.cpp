...
{
    uint32_t idx = (uint32_t)SERIE_INDEX(data->freq,RF_RX_FILTER_FREQ_STAR,RF_RX_FILTER_FREQ_STEP);
    m_data_f[idx] = *data;
}
...
{
    uint32_t idx = (uint32_t)SERIE_INDEX(data->freq,IF_RX_FILTER_160M_FREQ_STAR,IF_RX_FILTER_FREQ_STEP);
    m_data_f[idx] = *data;
}

void rx_if_fr_table::get(double freq,data_f_t *data)
{
    data->pwr = m_data_f[(uint32_t)(SERIE_INDEX(freq,IF_RX_FILTER_160M_FREQ_STAR,IF_RX_FILTER_FREQ_STEP))].pwr;
    data->freq = freq;
}
...
void rx_filter_80m_table::map2mem()
{
    ...
    uint64_t freq = 0;
    for (idx_m = 0;idx_m < (uint32_t)(RF_RX_FREQ_PTS_CALLED_R1C);idx_m ++) {
