...
int32_t for_0001_0002(cal_file *file)
{
    ...
    char path[64] = {0};
    char path_new[64] = {0};
    sprintf(path,"C:\\CSECal\\rfu%drf%d.cal",file->get_rfu_idx(),file->get_rf_idx());
    ...
    for (int32_t i = 0;i < cal_file_r1ab::X9119;i ++) {//for the list of calibration item is only allowed to accumulate,so it is safe to use the enum member here
    ...
    *strrchr(path_new,'\\') = 0;
    ...
    Log.add_msg_list(0,"file \"%s\" version 0x0001 ---> 0x0002",path);
    return 0;
}
