...
void tx_att_table_r1ab::get(uint64_t freq,int32_t att,double &offset)
{
    ...
    for (int32_t i = 0;i < (int32_t)(abs(att) / R1A_TX_ATT_STEP_MIN);i ++)
...
float tx_att_op_table_r1cd::get(uint64_t freq,double pwr)
{
    ...
    int32_t pwr_idx = SERIE_INDEX(i_pwr,R1C_TX_ATT_OP_POWER_STAR,R1C_TX_ATT_STEP);
    if (R1C_TX_BASE_POWER_OP < i_pwr)
        return data.offset[pwr_idx];
    else if (R1C_TX_BASE_POWER_OP == i_pwr)
		return 0.0;
	else
...
float tx_att_io_table_r1cd::get(uint64_t freq,double pwr)
{
    ...
    int32_t pwr_idx = SERIE_INDEX(i_pwr,R1C_TX_ATT_IO_POWER_STAR,R1C_TX_ATT_STEP);
    if (R1C_TX_BASE_POWER_IO < i_pwr)
        return data.offset[pwr_idx];
    else if (R1C_TX_BASE_POWER_IO == i_pwr)
		return 0.0;
	else
        return data.offset[pwr_idx -1];
}
