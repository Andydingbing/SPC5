...
bool basic_sp1401::g_en_expt = false;
...
bw_t basic_sp1401::get_bw()
{
    return m_cal_file->get_bw();
}

void basic_sp1401::set_expt_en(bool en)
{
    g_en_expt = en;
}

uint32_t basic_sp1401::get_rf_idx()
{
    return m_cal_file->get_rf_idx();
}

uint32_t basic_sp1401::get_rfu_idx()
{
    return m_cal_file->get_rfu_idx();
}

cal_file *basic_sp1401::get_cal_file()
{
    return m_cal_file.get();
}

vi_pci_dev* basic_sp1401::get_k7()
{
    return m_k7;
}

int32_t basic_sp1401::connect(vi_pci_dev *k7)
{
    m_k7 = k7;
    return 0;
}
...
int32_t basic_sp1401::set_sn(const char *hw_ver,int32_t ordinal)
{
    char sn[32] = {0};
    memset(sn,0,32);
    uint32_t rf_idx = m_cal_file->get_rf_idx();
    uint32_t rfu_idx = m_cal_file->get_rfu_idx();
    sprintf(sn,"%9s%08dRFU%dRF%d",hw_ver,ordinal,rfu_idx,rf_idx);
    INT_CHECK(w_eeprom(0x0000,24,sn));
    return 0;
}

int32_t basic_sp1401::set_sn(char *sn)
{
    INT_CHECK(w_eeprom(0x0000,24,sn));
    return 0;
}

int32_t basic_sp1401::get_sn(char *sn)
{
    INT_CHECK(r_eeprom(0x0000,24,sn));
    return 0;
}

arb_reader *basic_sp1401::get_arb_reader()
{
    return m_arb_reader.get();
}
...
int32_t basic_sp1401::w_eeprom(uint16_t addr,uint32_t length,char *buf)
{
...
int32_t basic_sp1401::set_pwr_mesa_trig_threshold(double if_pwr)
{
