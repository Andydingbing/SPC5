...
class RD_API tx_rf_fr_0_table_r1cd : public tx_rf_fr_table
{
    DECLARE_TX_RF_FR_TABLE_R1CD
};

// rf frequency response( cw 3100m ~~~ 6g )
class RD_API tx_rf_fr_1_table_r1cd : public tx_rf_fr_table
{
    DECLARE_TX_RF_FR_TABLE_R1CD
};

// if frequency response
class RD_API tx_if_fr_table
{
public:
    typedef struct RD_API data_f_t {
        double freq;
...
class RD_API tx_filter_table
{
public:
    typedef struct RD_API data_f_t {
        uint64_t freq;
        int16_t  coef_real[TX_FILTER_ORDER_2I];
        int16_t  coef_imag[TX_FILTER_ORDER_2I];
        int32_t  rsv_0;
        double   temp[4];
        tm       time;
        int32_t  rsv_1;
    ...
    STRUCT_ALIGN_API_S(data_m_t,4)
        sp_rd::sp1401::coef_complex_t coef[TX_FILTER_ORDER_2I];
    public:
...
class RD_API tx_filter_80m_table : public tx_filter_table
{
    ...
    DECLARE_CAL_ITEM_TABLE(tx_filter_80m_table,RF_TX_FREQ_PTS_CALLED)
};

// 160m
class RD_API tx_filter_160m_table : public tx_filter_table
{
    DECLARE_TX_FILTER_TABLE
    DECLARE_CAL_ITEM_TABLE(tx_filter_160m_table,RF_TX_FREQ_PTS_CALLED)
};

} //namespace sp_rd

#endif // TX_FILTER_TABLE_H
